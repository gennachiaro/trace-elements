...
pressures = pressures[pressures["Chosen"]== 1]
...
g = (sns.jointplot(data=df, x='Na2O', y='Pressures (MPa)', palette=color_dict, hue='Population', kind='kde', fill=True, joint_kws={'alpha': 0.7}, marginal_kws={
     'fill': True, 'common_norm' : False})).plot_joint(sns.kdeplot, zorder=0, alpha=0.3, warn_singular=False, linewidths=1).plot_joint(sns.scatterplot, style=df['Population'], s=40, markers = ('o', 's', '^', 'P', 'h')).set_axis_labels('Na2O','Pressure (MPa)')

#plt.xlim([680, 780])

g.ax_joint.scatter(data = MG3, x = 'Na2O', y = 'Pressures (MPa)', color = '#ABCFE5', marker = 'X', s=50, edgecolor = 'white', linewidth = 0.4, label = "CG 3")

# KDE plots for population MG 3
sns.kdeplot(data = MG3, x = 'Na2O', color = '#ABCFE5', fill = True, common_norm = False, ax = g.ax_marg_x)

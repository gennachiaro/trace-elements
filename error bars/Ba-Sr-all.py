...
FGCP = merge[merge['Population'].isin(['ORA-2A-016','ORA-2A-003','ORA-2A-023', 'ORA-2A-024'])]
...
FGCP_std = sample_std[sample_std['Population'].isin([ 'ORA-2A-016','ORA-2A-003','ORA-2A-023', 'ORA-2A-024'])]
# Drop bad samples
...
plot = sns.scatterplot(data = FGCP, x= x, y=y, hue = "Population", palette="Greens_r", style = "Population", edgecolor="black", s=150, legend = False, alpha = 0.8, hue_order = ['ORA-2A-003', 'ORA-2A-016', 'ORA-2A-023', 'ORA-2A-024'])

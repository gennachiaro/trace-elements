...
# Specify pathname
path = '/Users/gennachiaro/Documents/vanderbilt/research/ora caldera/EDS-Laser_Test/EDS_Laser_Test_PST.xlsx'
...
HSR = values[values['Population'].isin(['HSR'])]
MSR = values[values['Population'].isin(['MSR'])]
LSR = values[values['Population'].isin(['LSR'])]
Trachyte = values[values['Population'].isin(['Trachyte'])]

HSR_std = errors[errors['Population'].isin(['HSR'])]
MSR_std = errors[errors['Population'].isin(['MSR'])]
LSR_std = errors[errors['Population'].isin(['LSR'])]
Trachyte_std = errors[errors['Population'].isin(['Trachyte'])]
...
title = fig.suptitle("PST: Comparing EDS vs. LA-ICPMS Measurements (All)", fontsize=16, y=0.955)
...
xerr4 = Trachyte_std[x]
yerr4 = Trachyte_std[y]
...
plot1 = sns.scatterplot(data=HSR, x=x, y=y, hue="Sample", palette="PuRd_d", marker='s', style = "Location", edgecolor="black", s=150, alpha=0.8, legend= False)
plt.errorbar(x=HSR[x], y=HSR[y], xerr= xerr1, yerr = yerr1, ls='none',
             ecolor='cornflowerblue', elinewidth=1, capsize=2, alpha=0.8)

plot1 = sns.scatterplot(data=MSR, x=x, y=y, hue="Sample", palette="Blues_d", marker='s', style = "Location", edgecolor="black", s=150, alpha=0.8, legend= False)
plt.errorbar(x=MSR[x], y=MSR[y], xerr= xerr1, yerr = yerr1, ls='none',
             ecolor='cornflowerblue', elinewidth=1, capsize=2, alpha=0.8)

plot1 = sns.scatterplot(data=LSR, x=x, y=y, hue="Sample", palette="Blues_d", marker='s', style = "Location", edgecolor="black", s=150, alpha=0.8, legend= False)
plt.errorbar(x=LSR[x], y=LSR[y], xerr= xerr1, yerr = yerr1, ls='none',
             ecolor='cornflowerblue', elinewidth=1, capsize=2, alpha=0.8)

plot1 = sns.scatterplot(data=Trachyte, x=x, y=y, hue="Sample", palette="Blues_d", marker='s', style = "Location", edgecolor="black", s=150, alpha=0.8, legend= False)
plt.errorbar(x=Trachyte[x], y=Trachyte[y], xerr= xerr1, yerr = yerr1, ls='none',
             ecolor='cornflowerblue', elinewidth=1, capsize=2, alpha=0.8)

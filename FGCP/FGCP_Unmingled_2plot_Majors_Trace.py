...
all_FGUM = df[df['Population'].isin(['ORA-2A-003', 'ORA-2A-023'])]
...
FG1_std = sample_std[sample_std['Population'].isin(
    ['ORA-5B-410', 'ORA-5B-412', 'ORA-5B-414'])]
...
df1 = df1.drop(['Quartz Rim','Plagioclase Melt Inclusion', 'Quartz Rim', 'Glass (Quartz Rim)', 'Glass (LSR)'], axis=0)
df1 = df1.reset_index()
...
FGCP1 = sample_mean1[sample_mean1['Population'].isin(
    ['ORA-2A-003', 'ORA-2A-023'])]

FGCP1 = FGCP1.reset_index()
FGCP1 = FGCP1.set_index('Name')
...
FGCP1_std = sample_std1[sample_std1['Population'].isin(
    ['ORA-2A-003', 'ORA-2A-023'])]

FGCP1_std = FGCP1_std.reset_index()
FGCP1_std = FGCP1_std.set_index('Name')
...
FGUM1_All = df1[df1['Population'].isin(
    ['ORA-2A-003', 'ORA-2A-023'])]
...
plot = sns.scatterplot(data = FGUM1_All, x= x, y= y,hue = "Population", palette= "gray", edgecolor="black", marker = '^', s=150, style = "Population", alpha = 0.2, legend=False)

#CaO errorbar text
#plot.text(70.85,0.45, str('error bars $\pm$ 1$\sigma$'), fontsize = 11, fontweight = 'normal')

plot = sns.scatterplot(data=FGCP1, x=x, y=y, hue="Population", palette="Greens", style="Population", edgecolor="black",
                      s=200, legend=False, alpha=0.85)
plt.errorbar(x=FGCP1[x], y=FGCP1[y], xerr=xerr4, yerr=yerr4, ls='none',
...
plot = sns.scatterplot(data = all_FGUM, x= x, y=y, marker = '^', hue = "Population", style = "Population", palette="gray", edgecolor="black", s=150, alpha = 0.2, legend=False)
...
plot = sns.scatterplot(data=FGCP, x=x, y=y, hue="Population", palette="Greens", style="Population", edgecolor="black",
                       s=200, legend='brief', alpha=0.85)
plt.errorbar(x=FGCP[x], y=FGCP[y], xerr=xerr4, yerr=yerr4, ls='none',
             ecolor='green', elinewidth=1, capsize=2, barsabove=False, alpha=0.8)
...
#plt.savefig('/Users/gennachiaro/Documents/vanderbilt/research/ora caldera/trace-elements/graphs/FG_majors_trace_2panel.png', dpi=500)
